...
| | |
|---|---|
| **Conformance statement, exact** | **25 of 26 named guarantees implemented**, 0 failing, 0 skipped. **The one that is not is `G8`** — *"Orders after lockout are cancelled and logged"* — unimplemented since **2026-08-27**, when `a916bba` landed the decision [A11](AMENDMENTS.md) had taken the day before, removing cancel-on-observation after it cancelled the guardian's own flatten and **four orders of the trader's** — rejected twelve times between them, and the event carries no quantity, so whether any of them was reducing the position is not in the record. `G8`'s text is deliberately **not** rewritten to match the code, because the gap is real and rewriting it deletes the gap from view — [A12](AMENDMENTS.md). Not "it works", not "all tests green". *(A test COUNT used to stand here and was removed on 2026-09-01 rather than updated: the number climbs on its own, says nothing about coverage, and is wrong again the following week. A claim that expires by itself is a trap for whoever reads it next. The guarantee count is stable and checkable, so it is the one that stays — and since **2026-09-02 it is COMPUTED from the SPEC §15 table** by `C_ConformanceCountTests`, not typed. It read 26 of 26 for six days after `G8` stopped being implemented, because **nothing obliged a hand-written number to go down** — it could have been edited down at any moment on any of those days, and nothing made that happen.)* |
| **Exercised in production, exact** | **3 of 26 named guarantees exercised in production** — `G5`, `G14`, `G16`. A **second, harder number**, and it answers a different question from the one above: not *is it implemented* but **has the world ever made this rule do its job**. Computed since **2026-09-03** from `PRODUCTION EVIDENCE` markers in the SPEC §15 table by `C_ProductionExerciseCountTests`, never typed; each marked row cites the ledger `seq` where it was seen. **The marker is positive on purpose**: the default is *not established*, so this figure starts at 0 and can only rise when somebody measures a guarantee against the production ledger and says where. A negative marker would have been born at 26 and fallen only under effort nobody is obliged to spend — which is exactly how `26 of 26` survived six days. **`G3` and `G4` are measured and marked NOT exercised** ([`docs/g3-g4-episodio-20260826.md`](docs/g3-g4-episodio-20260826.md)); the other 21 are **not established**, which is not the same as passing. This is one ledger, on one machine, on a simulated account — and *exercised* is not *correct*: `G3`'s own defect is a rule that runs on a number its record cannot verify |
