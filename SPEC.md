...
| G5 — **PRODUCTION EVIDENCE** | Breach at exactly the limit trips (`>=`, not `>`) | P&L stream landing exactly on it. **PRODUCTION EVIDENCE**: fired on the boundary twice, at `seq 7150` (2026-08-26) and `seq 7995` (2026-08-31), both `LIMIT_BREACHED {dayLoss 40.00, limit 40.00}` — equal, not greater, so `>` would not have tripped either |
...
| G14 — **PRODUCTION EVIDENCE** | P&L sources disagreeing ⇒ `FAIL_CLOSED`, no tie-break | platform vs core mismatch. **PRODUCTION EVIDENCE**: `seq 3351` entered `FAIL_CLOSED` with reason *"SourcesDisagree on Sim101: core 0.00 vs platform -50.00 differ by 50.00, tolerance 5.00"*, cleared at `seq 6318` an hour later. One episode, and it took neither number nor the average of them |
| G15 | Missing price for an open position ⇒ `FAIL_CLOSED`, never zero | feed returns no quote |
| G16 — **PRODUCTION EVIDENCE** | Unknown/disconnected account ⇒ `FAIL_CLOSED` | account vanishes from the feed. **PRODUCTION EVIDENCE**: 12 episodes, each an `ACCOUNT_UNKNOWN` immediately followed by `FAIL_CLOSED_ENTERED {reason: "AccountUnknown on Sim101: account is Disconnected"}` — first pair `seq 28`→`seq 29`, last `seq 436`. Every one of them cleared by re-computation, never by assumption |
