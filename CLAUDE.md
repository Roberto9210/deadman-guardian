...
| **G3-INSUMO** — **el guardián actúa sobre un número que su propio registro no permite verificar** | **NUEVO 2026-09-03**, medido sobre el ledger de producción entero (8.106 filas, `docs/g3-g4-episodio-20260826.md`). **La aritmética del umbral SÍ está verificada**: G5 disparó en **40,00 contra 40,00**, exacto, y eso no está en discusión. **Lo no verificado es el INSUMO.** Dos escalones. **(1) El cotejo continuo compara sólo la mitad realizada**: `PnlAccounting.cs:277` compara `GrossRealized` contra `platform.GrossRealized`, y `Guardian.cs:710-711` publica exactamente esos dos. Ni lo no realizado ni las comisiones entran jamás. Y el −40,00 del 26-ago es **100 % no realizado** —`grossRealizedPerAccount` vale `0.00` en todos los checkpoints hasta el breach— tomado de la plataforma **sin cotejar**. **(2) Con cero de los dos lados cae a `Ok`**: `PnlAccounting.cs:288`, cuando la plataforma no reporta realizado el desacuerdo sólo se levanta si el de Core es distinto de cero ⇒ `0.00` contra `0.00` produce `PnlStatus.Ok`, **indistinguible de una coincidencia comprobada**. Los **0 `PNL_DISAGREEMENT`** del 26-ago no dicen que los números coincidieran: dicen que el chequeo no tenía nada que chequear. **Un error de comisiones nunca produciría un `PNL_DISAGREEMENT`.** **Forma del arreglo, anotada y NO hecha**: el ledger guarda **el total y no las partes**. Mientras no registre **realizado, no realizado y comisiones por separado**, un tercero no puede recomputar nada — sólo puede creernos. Eso entra en **el texto honesto del beta, no en el alcance.** | `PnlAccounting.cs:33,277,288` · `Guardian.cs:710-711` |
