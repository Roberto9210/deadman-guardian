...
| ~~cert-1 + cert-3~~ | **ARREGLADOS 2026-08-31.** El certificado se acota al día que nombra. **La definición**: el día empieza en la primera entrada con su  y termina donde empieza el día siguiente — o al final del registro si no hay siguiente. Derivable por cualquiera con una pasada, sin confiar en quien emite, y funciona sobre ledgers ya escritos. **NO es «entre  y »**:  se escribe ÚLTIMO en el armado, así que esa ventana excluiría el  que fija el límite.  se **cuenta** ahora (dayKeys distintos dentro del span), así que es verdad por construcción. Un día que el ledger no delimita se **rehúsa** (). **El primer borrador estuvo mal y lo atraparon cuatro C-tests existentes**:  sobre entradas con  termina en  en un día todavía ABIERTO, que es justo el que un trader exporta a las 16:55. **Hueco conocido y nombrado**:  precede a  sin , así que el  queda afuera; cerrarlo es un campo aditivo ⇒ contrato de extensión. ** NO se tocó**: es semántica y es decisión de producto. Tests: . |  |
